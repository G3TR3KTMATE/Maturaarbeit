...
# Bei RSA_Verschlüsslung wird der Code für die Umformung von Buchstaben zu Bytes importiert - Um auch Buchstaben zu Verschlüsseln

from tkinter import *
import tkinter as tk
import math
from RSA_Verschlüsslungs.Buchtstaben_zu_bytes import convert_to_num, convert_to_text
...
# Wird eingeführt, um das GUI bei Vergrösserung und Verkleinerung automatisch der Grösse der Fenster innerhalb des GUI's anzupassen
...
# Für das Fenster wenn man auf den Ja Knopf der Introfolie drückt - Neues Fenster
...
def open():
    ...
    # Für jeden Button, Text und Eingabefenster wird ein frame erstellt - Hilft beim Verändern der grösse des Fensters - Automatische anpassung mit der funktion "place"
    ...
    # Die 3 global's werden zusätzlich in anderen Funktionen gebraucht - Zwischenspeicher für andere Funktionen
    ...
    # Mit dem Button werden die 3 (global's) Eingaben abgespeichert und (command) ruft die Funktion open 1 auf - Neue Seite wird geöffnet
...
def open1():
    ...
    # Menü pic für einen Button, welcher einem zurück zum Einstellungfenster bringt

    menü_pic = PhotoImage(file="menü.png")
    background_image = tk.PhotoImage(file="Sunset.png")
    ...
    # Neue Frames füra GUI, welches aufgerufen wird (Seite 3)
    ...
    # Verschlüsslungsknopf - Wenn er gedrückt wird verweist er auf die Verschlüsslungsfunktion mit command=

    button_weiter = tk.Button(frame2, text="Verschlüsseln", bg="#00001a", fg="white", font=("Alegra", 30), bd=1,
                              command=Verschlüsseln)
    button_weiter.place(relwidth=1, relheight=1)

    # Entschlüsslungsknopf - Verweist auf Entschlüsslungfunktion mit command=
    ...
    # Button um das Programm nicht jedes Mal neu starten zu müssen, wenn man die Angaben der Primzahlen und des Startpunktes ändern möchte
    # Man kommt direkt auf das Menü Fenster - Neuspeicherung der Daten

    button_menü = tk.Button(frame5, image=menü_pic, bg="#00001a", fg="white", font=("Alegra", 30), bd=1, command=open)
    button_menü.place(relwidth=1, relheight=1)

    mainloop()


# Prüfung ob es sich auch um eine Primzahl handelt
...
def Verschlüsseln():
    ...
    # überprüfung von möglichen Fehlern , ein Error mit dem Text wird direkt im GUI anzeigt - der Error geht jedoch durch und somit stoppt das Programm
    # Ich weiss leider nicht wieso aber verschiedene Kombinationen mit Primzahlen unter 30 funktionieren nicht richtig und somit habe ich sie im Error File dazugefügt
    # Ich gehe von einem mathematischen Fehler aus aber kann es mir nicht ganz erklären
    # Das gleiche gilt, wenn die Message = m grösser ist als die Multiplikation der 2 Primzahlen
    # Dies ist jedoch momentan auf grau, da die Zahl m durchs umwandeltn mit utf-8 viel grösser wird als p*q
    ...
    # Berechnung von den Schlüsseln mit Hilfe des Startpunktes x
...
def Entschlüsseln():
    ...
    # Gleiches Prozedere wie bei Verschlüsseln - Die neue Eingabe wird mit m1 abgespeichert
...
# überprüfung der Gleichen Problemen wie Oben
# Falls jemands nur mit der Verschlüsung arbeiten würde ohne zuerst die Verschlüsslugsfunktion ausgelöst zu haben
...
# Hier kommt die eigentliche Entschlüsslungs ins Spiel - Umwandlung und danach die utf-8 Umformung zurück zum Ursprung
...
# Ich weiss es wäre nicht umbedingt eine nötige Seite des Programms, aber ich finde eine Startseite macht einen guten ersten Eindruck für das gesammte Programm
...
# Hier wird beim drücken des Knopfes die 2 Seite aufgerufen
